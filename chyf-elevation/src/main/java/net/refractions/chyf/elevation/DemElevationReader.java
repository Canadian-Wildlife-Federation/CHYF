/*
 * Copyright 2026 Canadian Wildlife Federation
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
package net.refractions.chyf.elevation;

import java.awt.Rectangle;
import java.awt.geom.AffineTransform;
import java.awt.geom.Point2D;
import java.awt.image.BufferedImage;
import java.io.File;
import java.io.IOException;
import java.net.URI;

import javax.imageio.metadata.IIOMetadata;

import org.geotools.api.referencing.crs.CoordinateReferenceSystem;
import org.geotools.api.referencing.operation.MathTransform;
import org.geotools.coverage.grid.GridCoverage2D;
import org.geotools.coverage.grid.GridCoverageFactory;
import org.geotools.coverage.grid.io.imageio.geotiff.GeoTiffIIOMetadataDecoder;
import org.geotools.coverage.grid.io.imageio.geotiff.PixelScale;
import org.geotools.coverage.grid.io.imageio.geotiff.TiePoint;
import org.geotools.gce.geotiff.GeoTiffWriter;
import org.geotools.geometry.jts.ReferencedEnvelope;
import org.geotools.referencing.CRS;
import org.geotools.referencing.operation.transform.ProjectiveTransform;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import it.geosolutions.imageio.core.BasicAuthURI;
import it.geosolutions.imageio.plugins.cog.CogImageReadParam;
import it.geosolutions.imageioimpl.plugins.cog.CogImageInputStream;
import it.geosolutions.imageioimpl.plugins.cog.CogImageInputStreamSpi;
import it.geosolutions.imageioimpl.plugins.cog.CogImageReader;
import it.geosolutions.imageioimpl.plugins.cog.CogImageReaderSpi;
import it.geosolutions.imageioimpl.plugins.cog.HttpRangeReader;

/**
 * Reads elevation values from a cloud-optimized GeoTIFF (COG) DEM,
 * returning the raster data for a given block as a {@link GridBlock}.
 *
 * @author Emily
 *
 */
public class DemElevationReader implements AutoCloseable {

	private static Logger logger = LoggerFactory.getLogger(DemElevationReader.class);

	private CogImageInputStream cogStream;
	private MathTransform worldToGrid;
	private AffineTransform gridToWorld;
	private CoordinateReferenceSystem gridCrs;
	private double scaleX, scaleY;
	private CogImageReader reader;

	private String COG_PATH = "";
	
	public DemElevationReader(String cogPath) throws Exception {
		this.COG_PATH = cogPath;
		init();		
	}

	public void close() {

		if (reader != null)
			reader.dispose();
		try {
			if (cogStream != null)
				cogStream.close();
		} catch (Exception ex) {
			logger.error("Unable to close DEM cog stream", ex);
		}
	}

	private void init() throws Exception {
		BasicAuthURI cogUri = new BasicAuthURI(URI.create(COG_PATH), false);
		CogImageReadParam param = new CogImageReadParam();
		param.setRangeReaderClass(HttpRangeReader.class);

		cogStream = (CogImageInputStream) new CogImageInputStreamSpi().createInputStreamInstance(cogUri, true, null);
		cogStream.init(param);
		reader = new CogImageReader(new CogImageReaderSpi());

		reader.setInput(cogStream);

		IIOMetadata metadata = reader.getImageMetadata(0);
		GeoTiffIIOMetadataDecoder decoder = new GeoTiffIIOMetadataDecoder(metadata);

		String projectedCode = "EPSG:" + decoder.getGeoKey(3072); // ProjectedCRSGeoKey
		gridCrs = CRS.decode(projectedCode, true);

		PixelScale pixelScale = decoder.getModelPixelScales();
		TiePoint[] tiePoints = decoder.getModelTiePoints();
		scaleX = pixelScale.getScaleX();
		scaleY = -pixelScale.getScaleY(); // negative — Y flips from image to geo
		double originX = tiePoints[0].getValueAt(3); // geo X of top-left pixel
		double originY = tiePoints[0].getValueAt(4); // geo Y of top-left pixel
		double tpPixelI = tiePoints[0].getValueAt(0);
		double tpPixelJ = tiePoints[0].getValueAt(1);

		gridToWorld = new AffineTransform(scaleX, 0, 0, scaleY, originX - tpPixelI * scaleX,
				originY - tpPixelJ * scaleY);
		worldToGrid = ProjectiveTransform.create(gridToWorld).inverse();

	}

	public GridBlock getElevations(Block block) throws Exception {

		logger.info("Loading DEM for block: " + block.getBlockId());

		ReferencedEnvelope re = block.getBounds();
		ReferencedEnvelope blockEnv = re.transform(gridCrs, false);
		
		ReferencedEnvelope gridEnv = new ReferencedEnvelope(blockEnv);
		// get more than needed to ensure we get overlapping areas
		gridEnv.expandBy(gridEnv.getWidth() * 0.01);

		double[] worldCoords = { gridEnv.getMinX(), gridEnv.getMinY(), gridEnv.getMaxX(), gridEnv.getMaxY() };
		double[] pixelCoords = new double[4];
		worldToGrid.transform(worldCoords, 0, pixelCoords, 0, 2);

		// GeoTIFF Y-axis is inverted vs image row order, so min/max need sorting
		int pixelMinX = (int) Math.floor(Math.min(pixelCoords[0], pixelCoords[2]));
		int pixelMinY = (int) Math.floor(Math.min(pixelCoords[1], pixelCoords[3]));
		int pixelMaxX = (int) Math.ceil(Math.max(pixelCoords[0], pixelCoords[2]));
		int pixelMaxY = (int) Math.ceil(Math.max(pixelCoords[1], pixelCoords[3]));

		Rectangle r = new Rectangle(pixelMinX, pixelMinY, pixelMaxX - pixelMinX, pixelMaxY - pixelMinY);

		CogImageReadParam param = new CogImageReadParam();
		param.setRangeReaderClass(HttpRangeReader.class);
		param.setSourceRegion(r);

		BufferedImage cogImage = reader.read(0, param);

		Point2D origin = new Point2D.Double(pixelMinX, pixelMinY);
		Point2D gridOrigin = gridToWorld.transform(origin, null);
		
		gridEnv = new ReferencedEnvelope(gridOrigin.getX(),
				gridOrigin.getX() + cogImage.getWidth() * scaleX, gridOrigin.getY(),
				gridOrigin.getY() + cogImage.getHeight() * scaleY, gridCrs);
		//writeToTestFile(cogImage, gridEnv);

		return new GridBlock(cogImage, gridEnv, block);

	}

	private static void writeToTestFile(BufferedImage image, ReferencedEnvelope envelope) throws IOException {
	    File output = new File("C:\\temp\\java_output.tif");

	    GridCoverageFactory factory = new GridCoverageFactory();
	    GridCoverage2D coverage = factory.create("dem", image, envelope);

	    GeoTiffWriter writer = new GeoTiffWriter(output);
	    try {
	        writer.write(coverage, null);
	    } finally {
	        writer.dispose();
	    }

	    System.out.println("Written to: " + output.getAbsolutePath());
	}

}